---
name: testing
description: "Use this agent to write and maintain tests for both backend (pytest) and frontend (Jest/Vitest). Covers auth tests, CRUD tests, filtering tests, validation tests, and test fixtures.\n\nExamples:\n\n<example>\nContext: The user wants to add tests for a new API endpoint.\nuser: \"Write tests for the GET /api/{user_id}/tasks endpoint\"\nassistant: \"I'll use the testing agent to write pytest tests covering auth, CRUD, filtering, and validation for the tasks list endpoint.\"\n<commentary>\nSince test writing is needed, use the testing agent which knows the test fixtures, mock JWT patterns, and test client setup.\n</commentary>\n</example>\n\n<example>\nContext: The user wants to verify auth middleware works correctly.\nuser: \"Test that invalid JWT tokens return 401\"\nassistant: \"I'll use the testing agent to write auth middleware tests covering missing, expired, and invalid token scenarios.\"\n<commentary>\nSince auth test coverage is needed, use the testing agent which has mock JWT token fixtures and knows the expected status codes.\n</commentary>\n</example>\n\n<example>\nContext: The user wants frontend component tests.\nuser: \"Write tests for the TaskList component\"\nassistant: \"I'll use the testing agent to write React Testing Library tests for the TaskList component covering empty state, rendering, and interactions.\"\n<commentary>\nSince frontend component testing is needed, use the testing agent which covers both backend pytest and frontend Jest/Vitest patterns.\n</commentary>\n</example>"
model: sonnet
color: purple
memory: project
---
